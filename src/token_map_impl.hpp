...
public:
  HostSet() {
    set_empty_key(Host::Ptr());
    set_deleted_key(Host::Ptr(new Host(Address("0.0.0.0", 0), false)));
  }
};
...
class DcMap : public sparsehash::dense_hash_map<uint32_t, Dc> {
public:
  DcMap() {
    set_empty_key(0);
    set_deleted_key(CASS_UINT32_MAX);
...
inline void build_datacenters(const HostSet& hosts, DcMap& result) {
  result.clear();
...
public:
  ...
  class DcRackInfoMap : public sparsehash::dense_hash_map<uint32_t, DcRackInfo> {
  public:
    DcRackInfoMap () {
      sparsehash::dense_hash_map<uint32_t, DcRackInfo>::set_empty_key(0);
    }
  };
  ...
  void build_replicas(const TokenHostVec& tokens, const DcMap& datacenters,
                      TokenReplicasVec& result) const;

private:
  void build_replicas_network_topology(const TokenHostVec& tokens, const DcMap& datacenters,
                                       TokenReplicasVec& result) const;
  void build_replicas_simple(const TokenHostVec& tokens, const DcMap& datacenters,
                             TokenReplicasVec& result) const ;
  void build_replicas_non_replicated(const TokenHostVec& tokens, const DcMap& datacenters,
                                     TokenReplicasVec& result) const;
...
template <class Partitioner>
void ReplicationStrategy<Partitioner>::build_replicas(const TokenHostVec& tokens, const DcMap& datacenters,
                                                      TokenReplicasVec& result) const {
  result.clear();
...
template <class Partitioner>
void ReplicationStrategy<Partitioner>::build_replicas_network_topology(const TokenHostVec& tokens, const DcMap& datacenters,
                                                                       TokenReplicasVec& result) const {
  if (replication_factors_.empty()) {
    return;
  }

  DcRackInfoMap dc_racks;
  ...
  for (ReplicationFactorMap::const_iterator i = replication_factors_.begin(),
    ...
    DcMap::const_iterator j = datacenters.find(i->first);
  ...
  for (typename TokenHostVec::const_iterator i = tokens.begin(),
    ...
    for (typename DcRackInfoMap::iterator j = dc_racks.begin(),
    ...
    for (typename TokenHostVec::const_iterator j = tokens.begin(),
      ...
      typename DcRackInfoMap::iterator dc_rack_it = dc_racks.find(dc);
      if (dc_rack_it == dc_racks.end()) {
        continue;
      }

      DcRackInfo& dc_rack_info = dc_rack_it->second;
...
template <class Partitioner>
void ReplicationStrategy<Partitioner>::build_replicas_simple(const TokenHostVec& tokens, const DcMap& not_used,
                                                             TokenReplicasVec& result) const {
...
template <class Partitioner>
void ReplicationStrategy<Partitioner>::build_replicas_non_replicated(const TokenHostVec& tokens, const DcMap& not_used,
                                                                     TokenReplicasVec& result) const {
...
template <class Partitioner>
void TokenMapImpl<Partitioner>::update_host(const Host::Ptr& host, const Value* tokens) {
...
template <class Partitioner>
void TokenMapImpl<Partitioner>::remove_host(const Host::Ptr& host) {
  internal_remove_host(host);
  build_replicas();
  hosts_.erase(host);
}
...
template <class Partitioner>
void TokenMapImpl<Partitioner>::update_keyspaces(const VersionNumber& cassandra_version,
                                                 ResultResponse* result) {
...
template <class Partitioner>
void TokenMapImpl<Partitioner>::internal_update_keyspace(const VersionNumber& cassandra_version,
                                                         ResultResponse* result,
                                                         bool should_build_replicas) {
...
template <class Partitioner>
void TokenMapImpl<Partitioner>::internal_remove_host(const Host::Ptr& host) {
